...
- **Gates you can press.** The workflows come with a Claude Code mod: while the terminal is the only screen attached, every menu is drawn as buttons above the prompt, where they stay put while the transcript scrolls — a click picks a row, and a second click or Enter sends it. The first run switches it on in the project's settings and asks you to restart Claude Code once.
