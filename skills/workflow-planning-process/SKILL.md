...
5. Delete the planning manifest entry:
   ...
   ```
6. Commit with raw git — the planning item was just deleted, so `--plan` has nothing to read; stage the work unit and the plan's `storage_paths` (from the manifest subtree read during resume detection), then commit: Expand the array to bare pathspecs (`[\".tick/\"]` → `.tick/`); when it is `[]`, stage nothing extra.
   ```bash
