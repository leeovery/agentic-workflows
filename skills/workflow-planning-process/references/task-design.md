...
A task whose edits land on another work unit's specification under `.workflows/` corrects a historical artifact. Fold the completed-unit protocol from **[correcting-historical-artifacts.md](../../workflow-shared/references/correcting-historical-artifacts.md)** into the task itself — in-place edit, corrigenda entry, knowledge re-index, scoped commit — as **Do** steps and **Acceptance Criteria**. The re-index is part of the task, never an afterthought. No task edits another work unit's research, discussion, or investigation files — those decay in the knowledge base and are superseded by current work, never corrected.
