...
An item that was once a topic has a second record — the epic that postponed it. Read `engine manifest get project.roadmap.items.{name}.postponed_from`; a value is `{work_unit, topic}`, and the read set derives from that address, never from `sources`. In this order, every path relative to `.workflows/{work_unit}/`: the brief (`engine manifest get {work_unit}.discovery.{topic} brief_path`, canonically `discovery/briefs/{topic}.md`), `research/{topic}.md`, `discussion/{topic}.md`, then every file under `research/.triage/{topic}/` and `discussion/.triage/{topic}/`. Read in full whatever exists. The prior epic's files are its record: read them, never edit, drain, or move them.
