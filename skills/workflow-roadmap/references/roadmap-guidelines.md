...
- An add aimed at a horizon with **any member in delivery** takes the routed confirm (`engine render roadmap-add-gate --horizon {h}` — reason from its DATA, emit its MENU section verbatim per its marker, then STOP for the answer). While waiting members remain the menu is three-way; once the horizon is fully in delivery it is strict two-way — no waiting side-door into a release that is now an epic. Route the answer:
