...
The register for the task loop's report moments — the task brief, the findings summary, the result summary, and the technical retells. A report is sectioned facts, not narrative: the user takes the state of the work in at a glance, and depth is pulled, not pushed — the technical retell, **Ask**, and the record files sit one option away at the gate. The record files stay fully technical and remain authoritative — a report presents them, never replaces them.

The register governs report blocks only. Conversational turns inside the loop — Ask answers, comment exchanges, blocker discussion — follow **[voice.md](../../workflow-shared/references/voice.md)** as everywhere else. Engine-emitted `DISPLAY`/`MENU` sections sit outside it entirely, byte-for-byte. Agent reports, cache files, and records are never written in it — the register applies when composing for the user, never to what lands on disk.
...
- **No narrative framing.** No "worth noting", "on the way", "three things". A fact sits under its section label.
- **Lists for parallel facts.** Enumerations go vertical. A section with nothing to say is omitted, never padded.
...
Audience follows the lens: the task brief, findings summary, and product summary translate codebase-internal names into what they do; the technical retell uses real names with `file:line` anchors.
...
Markdown, not a code block. Issues in severity order. Per issue, a short block: what is wrong in what was built, the risk it carries, and the proposed fix. Include the alternative or the reviewer's confidence only where it changes the call. Non-blocking notes: one line each.
...
A retelling, not a summary: every substantive point in the record appears, nothing softened, nothing dropped. On the fix gate, the same decomposition applies to the findings — the mechanism of each issue and where it sits.
