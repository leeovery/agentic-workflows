...
`{task_number}`/`{task_total}` are the task's ordinal and the plan's task total, noted at **A. Retrieve Next Task** — re-derive them from the format's **reading.md** listing when they are not in session context. When the format cannot yield them, the marker carries the name alone: `**`▪ {Task Name}`**`.
