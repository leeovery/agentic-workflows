...
6. **Resume at the stage that shows the moment.** Every gate menu and header is served by its own render surface, fetched by the stage that displays it — the task verbs answer with JSON only, and re-running one re-emits nothing. Once the position is confirmed, re-enter that stage and let it fetch its sections: a pending fix gate at **F. Fix Approval Gate** and a pending task gate at the gated fetch in **G. Task Gate** (**[task-loop.md](references/task-loop.md)**), a pending conclude gate at **Step 9**; a presentation moment re-runs its display reference (**[display-task-brief.md](references/display-task-brief.md)**, **[display-task-result.md](references/display-task-result.md)**), which rebuilds its payload before rendering. Never run `fix-attempt` or `analysis-cycle` to reconstruct position — each records a new attempt or cycle.
