...
- Do not invent stops. Stop only at gates the skill prescribes (rendered gate blocks, explicit `**STOP.**` directives). Do not insert courtesy check-ins, "ready to continue?" prompts, mid-loop summaries that end the turn, or pauses between tasks/topics/phases that the skill does not prescribe. The phase runs continuously between defined gates. See **[no-artificial-stops.md](../workflow-shared/references/no-artificial-stops.md)**.
- Failure mode — "this is a lot, the user might want a checkpoint": that thought is not a gate. Continue.
- Failure mode — "I should check in before the next task/topic/phase": only if the skill prescribes a gate there. Otherwise continue.
