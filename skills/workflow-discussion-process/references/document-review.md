...
Walk the conversation against the document and check six dimensions:
...
6. **Pipeline meta** — the document stating its own pipeline position: readiness declarations ("ready for specification"), decided-subtopic counts, review-cycle tallies — whether written this session or inherited from an earlier one. The manifest carries that state; as prose it rots. Usually lands in Summary → Current State, but check everywhere.
...
- Pipeline meta → strike it — rehome any genuine substance it carries (an open condition becomes an Open Thread), never the status itself
