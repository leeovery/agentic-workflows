...
**Concrete before abstract**: Lead with a worked instance, not a description of a mechanism. Show the case — specific values, named actors, an actual sequence — then generalise. A reader who can picture the failure can judge the fix; a reader parsing a mechanism is still building the picture when the question arrives.
