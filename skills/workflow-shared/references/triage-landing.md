...
Set `landed_topic = {target}` and `result = landed`. When the response carries `reconcile_flagged`, the landing flagged completed downstream work to reconcile — on a research-side landing, the target's completed discussion (the landing reopened its ground); on a discussion-side landing, the specification(s) sourcing the target, named in `sources_staled`, whose extracted rows are now `stale`. The caller's landing line should say which.
