...
2. **Reindex it**: `node .claude/skills/workflow-knowledge/scripts/knowledge.cjs index {the resolved artifact path}` — the knowledge base serves the resolution for the rest of the work. A failed index never blocks: tell the user in one line that the next start retries it, and continue.
...
4. **Commit**: `node .claude/skills/workflow-engine/scripts/engine.cjs commit {work_unit} -m "{source_phase}({work_unit}/{doc}): {what the resolution settled}" --topic {source_phase}/{doc} --sweep`. `--sweep` says the topic is somebody else's.
