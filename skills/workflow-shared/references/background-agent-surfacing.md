...
3. Digest the finding from the content file — never read it out — and compose the raise in three beats. A findings walk is a series of cold starts — each raise lands in a corner of the document the user last held fully hours or days ago — so the first beat rebuilds that context rather than referencing it:
   - **Present** — scene reconstruction before any assessment. Say where it came from (the background {agent_type}) and what it observed — for a synthesis, the two positions in tension. Entering a new scene, rebuild it concretely: establish the normal case with specific values, show it resolving fine, then perturb it to expose the finding — the technical detail comes after the instance lands. Use whatever device fits — a worked scenario, a small diagram, a before/after list, light headings — and vary the device across the walk. Within a scene already rebuilt this session, or when the finding set was visible moments ago, a bridge clause suffices. The test: the user can picture the problem before the ask arrives. Restate any term borrowed from another subtopic or an earlier decision; never reference it bare. Never use a bare id (`F5`, `T2`) as a label in conversational prose — name the finding by its report title on first mention, or describe it by what it is; ids belong in commit subjects (`(review-003 F5)`) and in-document markers (`(resolves review-003 F5)`), not in the conversation. When earlier findings from this set have been raised, open with a one-line bridge: what the previous one settled — or simply that it was raised, when that engagement predates this session — and how many follow this one (the surface response's `remaining`; `findings` is the full set).
