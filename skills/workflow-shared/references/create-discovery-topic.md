...
*Shared reference. Loaded by `workflow-research-process` (topic spawning) and `triage-landing.md` (reroute to a new topic), and any flow that spawns a new discovery-map topic.*
...
- `source` — the provenance string for the discovery item (e.g. `spawn:{parent}`, `reroute:{origin}`).
