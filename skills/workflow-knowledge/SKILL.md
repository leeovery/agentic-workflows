...
- `buildable` — the project is set up (its committed `.workflows/.knowledge/config.json`) but this checkout has no store, and this machine's config says how to build one: a provider whose key resolves, or keyword-only chosen outright — a system config holding knowledge settings that name no provider, or a project config unsetting the provider. No system config, an invalid one, or a provider whose key cannot be resolved is never read as a keyword-only choice
...
- Exits non-zero if the file doesn't exist, the path can't be parsed, or the path names a non-markdown import — and, either form, when there is no store to write and the configured provider's key cannot be resolved: no store is created in keyword-only mode in its place.
...
- **`--keyword-only`** — project-level keyword-only init: project config, empty store, provider-null metadata. Never touches the system config — the project config always pins `provider: null`, so the choice travels with the project: it runs keyword-only whatever a machine's system config names, and a checkout with no store is `buildable` keyword-only on any machine. Idempotent; partial states fill in; a store without metadata refuses toward `knowledge rebuild`.
