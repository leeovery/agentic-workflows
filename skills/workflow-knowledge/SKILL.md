...
- A rate-limited request (HTTP 429) waits the time the provider names — capped at a minute, growing waits when it names none — and is retried alone, up to five times; an account out of quota fails at once. Other transient failures (network, server error, lock, store I/O) are retried with exponential backoff; a rejected request (HTTP 400/413/422), a bad key, or a provider configuration the model contradicts fails at once. A file that still fails exits non-zero — the single-file form says the next start retries it; the bulk index attempts every file — embedding everything new and changed in one batch, file by file when the endpoint refuses an input in it (so the refused file fails alone), and failing every file in the batch on any other failure — names each failure on stderr, and exits non-zero at the end. A manifest the bulk index cannot read aborts it before anything is removed.
