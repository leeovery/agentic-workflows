...
Dependencies are stored in the **manifest** as `{work_unit}.phases.planning.external_dependencies`. See [dependencies.md](dependencies.md) for the format and states.
...
1. **Document each dependency** in the manifest's `{work_unit}.phases.planning.external_dependencies` field using the format described in [dependencies.md](dependencies.md). Initially, record each as `state: unresolved`.
