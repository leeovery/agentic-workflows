...
Check the `review_cycle` field via manifest CLI (`node .claude/skills/workflow-manifest/scripts/manifest.js get {work_unit}.phases.specification.review_cycle`).

#### If `review_cycle` is 0 or not set

Set `review_cycle` to 1 via manifest CLI (`node .claude/skills/workflow-manifest/scripts/manifest.js set {work_unit}.phases.specification.review_cycle 1`).

#### If `review_cycle` is already set

Increment `review_cycle` by 1 via manifest CLI (`node .claude/skills/workflow-manifest/scripts/manifest.js set {work_unit}.phases.specification.review_cycle {N}`).
...
- **Source material paths**: all source document paths from the manifest (`node .claude/skills/workflow-manifest/scripts/manifest.js get {work_unit}.phases.specification.sources`)
