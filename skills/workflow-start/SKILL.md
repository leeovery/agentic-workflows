...
```
> The workflows come with a Claude Code mod, an experimental feature, now switched on in this project's `.claude/settings.json`. It shows each menu as buttons above the prompt — click a row to pick it, click again to send — and in a workflow session it makes sure what the workflow shows you reaches the screen as written. Your other conversations in this project keep Claude Code's defaults and your own settings.
```
