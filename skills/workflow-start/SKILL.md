...
Branch on the boot response — run no further commands (the bulk `knowledge index` and `compact` already ran inside boot when the knowledge base was ready). If it carries `warnings`, surface them and continue — boot is complete; the next start retries what failed.
