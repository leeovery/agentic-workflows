...
`Declined` records a finding left as-is with the reason in Notes: the outcome of the gate's Discuss exchange, or a preference nothing leans on declined at dispose — no side costs the user, so the specification states no rule for it. It is never offered as a menu row — a decline without a stated reason is a skip whatever it is called.
