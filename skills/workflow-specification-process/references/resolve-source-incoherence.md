...
5. **Commit**: `node .claude/skills/workflow-engine/scripts/engine.cjs commit {work_unit} -m "{source phase}({work_unit}/{doc}): {what the resolution settled}" --topic {source phase}/{doc} --kb --sweep`. `--kb` carries the reindex; `--sweep` says the topic is not this session's — the document belongs to `{doc}`'s own phase, and an idle-but-alive session may still hold it.
