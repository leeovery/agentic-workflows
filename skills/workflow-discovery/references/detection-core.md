...
**Pre-seeded vs open start.** A `workflow-start` menu pick pre-seeds the work type — start at high confidence; the loop mostly *confirms* (watch for disconfirmation, then move). `s`/start pre-seeds nothing — the loop *establishes*. Same discipline; depth differs.
...
If the user accepts, invoke the matching capture skill (`/workflow-log-idea`, `/workflow-log-bug`, or `/workflow-log-quickfix` — default to idea if unsure) and note the surfacing so it's recoverable. Continue with the original work, now without scope creep. If the user says it's actually part of this work, fold it in. Soft, conversational — no structured gate.
