...
2. **Recognise intent.** The user's message may contain:
   ...
   - **A request to see the map** — *"show map"*, *"what's on the map"*. Re-run `gateway.cjs map-view {work_unit}` and emit its `=== DISPLAY` section verbatim as a makefile code block (```makefile fence). No STOP gate; just render and continue.
