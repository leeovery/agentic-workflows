...
- **[commands.md](references/commands.md)** — the CLI catalogue: command grammar, the response contract, and every noun's full signature and behaviour (`boot`, `manifest`, `workunit`, `topic`, `sources`, `discovery-map`, `discovery-session`, `discussion-map`, `task`, `inbox`, `cache`, `presence`, `commit`).
