...
**Sessions run concurrently, so every write is confined.** Manifest writes hold the work unit's lock; cache state is partitioned per topic, bar the project-level session-label store, keyed per tmux session (its positions per Claude session id) and written atomically; and every engine commit — the `commit` helper and every transaction tail — commits a pathspec (boot's untracking of the knowledge store, a scratch index of HEAD without those paths), never the index, so no commit can reach outside the paths its own action wrote. Session liveness rides the same principle: heartbeats are stamped by the engine as a side effect of the verbs a session runs on its own topic (`presence`), never by prose.
