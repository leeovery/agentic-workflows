...
const { openGate, section, titleSection, CONTINUE_INSTRUCTION, CONTINUE_MARKDOWN_INSTRUCTION, AUTO_GATE_INSTRUCTION, AUTO_GATE_MARKDOWN_INSTRUCTION, menu, menuFrame, MENU_GLYPH, cmdOption, bareOption, promptOption, callout, indentedBody, bulletRow, subDetail, treeList } = require('./projections/surfaces.cjs');
...
function externalDependencyGate(cwd, { dotpath, variant, blocking }) {
  ...
  const rows = names.map((name, i) => {
    ...
    return cmdOption(String(i + 1), null, { head: `${titlecase(name)} — ${dep.description}` });
  });
