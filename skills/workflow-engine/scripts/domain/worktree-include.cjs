...
// ---------------------------------------------------------------------------
// Domain ring: the knowledge store's copy into new worktrees. Claude Code
...
// keeps the store's files listed there and a new worktree starts with the
// index its checkout already built. The lines are appended when missing;
// every other line in the file is the user's and stays as it is.
// ---------------------------------------------------------------------------
...
/**
 * Ensure `.worktreeinclude` lists the store's files, creating the file when
 * there is none. A file that cannot be read or written is left as found and
 * reported rather than thrown: boot must never fail over it.
