...
- **One primary question.** Every experiment answers one question; anything else it measures is explicitly secondary. Two primary questions are two experiments.
...
- **Every number traces to a file or a named source.** A result that can't be traced from the report to the data that produced it is the failure mode this phase exists to prevent.
...
Experiments measure; discussions decide. The verdict is the decision rule's mechanical outcome, and the topic's discussion — which reads the report as evidence — can override it. When a result tempts you toward "therefore we should…", that is evidence ready for discussion, not a conclusion for the report's reading.
