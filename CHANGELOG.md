...
🔧 Changed
- Interrupting a phase to backlog an idea or cancel a topic now re-presents any gate that was pending on return, instead of silently dropping it.
- The `storage_paths` check across planning, implementation, review, and scoping now correctly treats an empty array as present, only triggering on a genuinely missing field.
...
🔧 Changed
- Roadmap prompts, menus, and confirmations now speak in plain words instead of workflow jargon like "pull", "slice", "fence", and "commit" — items are now "started" or "brought over" rather than "pulled into delivery".
- New worktrees now copy `node_modules/` in automatically instead of requiring a fresh install.
...
🔧 Changed
- Merged the "proceed to review" and "revisit an earlier phase" prompts between pipeline phases into one combined gate.
- Concluding an implementation or plan now offers to answer questions about it instead of a dead-end "no".

🐛 Fixed
- Resuming a session after implementation analysis converged with no new findings no longer gets stuck re-entering the analysis loop.
...
🔧 Changed
- Reference hand-offs in the discussion, research, and specification skills now explicitly resume the calling step after returning, preventing a session from stalling mid-flow.
- The next raise after an absorb now opens by naming what the previous one settled and how many remain.
...
🔧 Changed
- The convergence diagnostic display now wraps its heading, resolved/recurring/new-cycle lists, and hypothesis notes to the terminal width instead of overflowing narrow panes.
...
- The asserter now reads its prompt from a file in the test world instead of receiving it relayed by the orchestrator — long walk records no longer get silently truncated before judging.
...
🔧 Changed
- Session labels now stash their original tmux name per checkout (`.workflows/.cache/.session-labels/`) instead of a machine-global store — restores no longer collide across projects sharing the same tmux server.
- The SessionEnd hook resolves the project root the same way for both presence and session-label cleanup, so cleanup fires reliably even when the hook runs from a drifted working directory.

## [0.7.41] - 2026-09-12

🔧 Changed

- Planning now settles its own how-forks — phase ownership, task grouping, internal bounds — on the planner's honest call instead of stopping to ask, reserving stops for genuine gaps in what the product does.
- Traceability review checks only that product content ties back to the spec, no longer flagging planner-owned implementation mechanics as hallucinated.
...
🐛 Fixed
- Resolved source documents could still leak references to the specification, a review, or a tracking file inside a new decision's own Decision line — the rule now covers every sentence, parenthetical, and reference in the section, not just the surrounding prose.
...
🐛 Fixed
- Review reports no longer silently pass phase acceptance criteria that were never actually measured — unmeasured criteria are now explicitly disclosed instead of checked off.

🔧 Changed
- The review process's change-set cycle count is now derived with an explicit command, correctly reading zero for a topic with no prior review cycles.
...
🔧 Changed
- An abandoned or concluded experiment's awaiting note now records a dated entry when the wait settles, instead of leaving stale present-tense text behind.

## [0.7.26] - 2026-09-01

🐛 Fixed
- Presence reads (`topic queue`, `agent scan`) on another session's topic no longer manufacture a false hold — they now only refresh a heartbeat this session already owns.
...
🔧 Changed
- Spec source-repair now revises a Decision block's dated timeline entry against the failed measurement instead of overwriting its recorded value in place.
...
🔧 Changed
- Sibling-knowledge checks in research and discussion reviews now recognize deferred/triaged questions as settled, so leans a sibling topic already deferred no longer get flagged as owed corrections.

## [0.7.18] - 2026-08-30

✨ Added
- Linter setup now handles projects with no candidate linters — proceeds without linting during TDD instead of stalling.

## [0.7.17] - 2026-08-30

🔧 Changed
- Confirming a previously-approved set of project skills or linters now shows a compact count-and-name list instead of the full detail table, since it's already been approved once.
- Re-confirming project skills or linters at topic level no longer re-asks or clears the stored set — it's reused directly.
...
- Wrong-side triage concerns can now be moved to the topic's other phase-side (research↔discussion) instead of being worked where they landed.

🔧 Changed

- A correction lands wherever the source material was already recorded, not automatically on the discussion side.

## [0.7.10] - 2026-08-24

🔧 Changed
- Specification review findings now inline the substance of a referenced section instead of citing its number — findings read clearly without needing to open the spec.

## [0.7.9] - 2026-08-24

🔧 Changed
- Walk raise push-back invitations now point at the reason already given in the opener instead of introducing a new pivot point.
...
🔧 Changed
- Review "Corrected in this session" reports now derive from the apply commit and diff instead of trusting a status claim, so the record matches what was actually applied.
- A do-now apply with nothing successfully applied now reports zero applied with reasons instead of writing a commit.
...
🐛 Fixed
- Task commit subject format clarified with a concrete example — prevents malformed commit tokens that break review's task-detection.
...
✨ Added
- Task loop results now render as a single header — verdict, fix rounds, phase, and position — shown at every approved, needs-changes, blocked, and failed moment.
- Tick-format plans show the tick id alongside the internal task id in task result displays.

🔧 Changed
- The old fix-threshold callout is folded into the new result header instead of appearing as a separate line.
- Task and fix approval prompts drop the redundant task id since the result header above them already names the task.

## [0.6.55] - 2026-08-10

🔧 Changed

- Spec review status is now held in session context rather than written to the manifest — phase statuses stay ephemeral until the branch below decides what to do with them.
...
✨ Added
- Task reviewers now flag stale or incorrect code comments as standalone corrections, applied automatically without triggering a fix round.
- The comment discipline for implementation work is now documented — what earns a comment, what never belongs in one, and when to fix a stale one.
- Task authors are now barred from dictating comment text in task steps — comments stay the executor's call, keeping rationale in the task record instead.

🔧 Changed
- The rerouted-concerns triage queue now renders as a numbered list with per-item origin details instead of a flat callout.
...
🐛 Fixed
- Drain-triage and document-review concern presentations now render as formatted markdown instead of a plain code block.
- Concern breakdowns in drain-triage now walk through the problem in plain language before asking the user anything, instead of jumping straight to a question.
...
🐛 Fixed
- Bare finding/tension ids (like `F5` or `T2`) no longer show up as labels in background-agent conversational prose — findings are now named by their report title instead.
...
🔧 Changed
- Planning loops that resume after the last phase already completed now finish cleanly instead of stalling.

## [0.6.16] - 2026-07-28

🐛 Fixed
- Spec review now resolves a source referencing its own topic to the correct artifact instead of misrouting it as an incorporated spec.
...
🔧 Changed
- Completing, cancelling, or absorbing a work unit now clears its scratch cache — reactivating one rebuilds it on demand.
- Interactive menus, gates, and status messages are now generated by the same deterministic engine core as the dashboards, so what you see always matches the real state.

🐛 Fixed
- Existing installs get a one-time migration that purges leftover scratch caches from work units closed before this cleanup existed.
