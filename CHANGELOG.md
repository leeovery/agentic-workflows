...
- Treat a completed implementation as satisfying a resolved dependency even when the referenced task was skipped or its ID is stale (applied in both the epic dashboard and the implementation-entry dependency check)
- Add test coverage for external dependency blocking, satisfaction, and next-phase-ready gating in epic discovery
