...
- **R6 — Open is a fine way to conclude.** The conclude gate shows the register and asks the same question it asks today. Open, being-dug, and parked threads land in the research file's Open Threads, which discussion reads in full — a dive that never landed is named as such. No thread state blocks the close.
...
**The close** — the fold of every landed dive and the in-flight gate (wait or proceed) → triage queue → waits → document review → compliance → the register rendered above the conclude gate. The conclusion writes open, being-dug, and parked threads into the file's Open Threads. No review, no movement judgment, no decline to remember.
...
- **PR4 — shared references re-homed.** `final-review-menu.md`, `background-agent-surfacing.md`, and `composing-a-raise.md` move into `workflow-discussion-process/references/` — discussion alone loads them; `natural-breaks.md` stays shared, loaded explicitly by `rerouted-concerns.md` (whose triage check consulted it without loading it), and `making-it-land.md` stays shared, loaded by the implementation phase's decision raise; every loader path updated; `agent_type` narrows to `review` | `synthesis`; attribution lines take the single-parent form; every prose case `files` list naming a moved file updated. A reference another skill still loads stays shared.
...
- 2026-09-12 — Second review pass: the close hands a fold that ended on a question back to the loop, so a second landed report is never concluded over; a held-back thread is re-offered from the fold's no-pending exit; rerouted and grown threads move the register at the reference that lands the concern, children travelling with a grown thread; absorb copies a phase item whole — the Discussion Map included — so no field list can drop state again; an origin is one line; the triage caller, not the shared checklist, rules that a conclusion attempt beats a recent `later`; the perspective offer is an engine surface.
