...
Refused when: the work unit does not exist or is not `in-progress`; `--from` is absent, not a valid origin, `roadmap` (the product layer's own, with its own verb), or a `{phase}/{topic}` naming no phase item; any path is missing or is not a regular file (the whole call fails with `missing_imports` on the payload, nothing copied — a dropped folder never half-lands); every source skips on normalisation. In one lock hold: plan and dedupe against the directory and the batch, copy each file (mode `0644`, whatever the source's), push `{path, imported_at, origin}`. After the lock: `knowledge index` for each landed `.md` (warn-don't-block); `commitTailWithKb` over `.workflows/{wu}/imports`, `.workflows/{wu}/manifest.json`, and the store dirt, message `workflow({wu}): import {n} file(s) for {origin}`, the generic retry note (no narrower `engine commit` scope exists, and a note naming the work unit would prescribe the sweep the confinement exists to prevent). The dispatch beats presence on `--from`'s phase and topic after the domain call, as `topic absorb` does, and only while that item is not terminal; a bare `discovery` origin beats nothing. Response: `{ op: 'import', imports: [{path, origin}], skipped_imports, warnings?, commit }`.
...
- **`workflow-knowledge/SKILL.md`, `CLAUDE.md`, and `commands.md`** stop saying imports arrive only at the opener and are always indexed; the presence beat table and `CLAUDE.md`'s presence list name the verb.
...
| `domain/workunit-import.cjs` (new) | `importWorkUnitFiles(cwd, wu, paths, {origin})` — refusals (including a `roadmap` origin and an origin naming no item), the lock hold, index, the confined tail commit with the generic retry note |
...
| `engine.cjs` | `workunit import` dispatch with `beatQuietly` on the origin while its item is live; usage lines |
| `render.cjs`, `projections/surfaces.cjs` | `render import-reprompt --file` (the confirm-trigger's missing-imports re-prompt, engine-rendered); `render promote-receipt --imports <N>` |
...
- `test-engine-workunit-import.cjs` (new) — happy path and the beat over every `IMPORT_PHASES` origin and bare `discovery`; the entry shape; `0644` on the copy; refusals (missing unit, completed unit, bad origin, `roadmap`, a leading-dash topic, an origin naming no item, missing path or a directory copies nothing, all-skipped, a leading-dot source skipped); dedupe against the directory and the batch; the confined commit stages the imports directory and the manifest and nothing else dirty; KB failure warns and the commit still lands; the generic retry note on a failed commit; no beat on a terminal item.
