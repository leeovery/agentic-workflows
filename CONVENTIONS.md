...
**A menu whose option set varies at runtime is engine-rendered, never authored.** Hand-carried padding is fixed when the file is written, so a row behind a conditional cannot be aligned for both states at once — the column is wrong whenever the set differs from the one the author counted. Build the options in code and let `menu()` compute the column for whichever set survives (`workflow-engine/scripts/domain/projections/surfaces.cjs`). Static menus stay in prose: their key column never moves, so hand-carried padding is correct by construction.
