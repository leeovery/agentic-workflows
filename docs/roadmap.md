...
Items also arrive from work already under way. A topic inside an epic that turns out to belong to a later release is **postponed** onto the roadmap rather than cancelled (see [lifecycle operations](lifecycle-operations.md)): it leaves the epic whole, waits here under the horizon you name like any other item, and comes back the way everything else does — by being pulled. Pulled back into the epic it came from it picks up exactly where it stopped; pulled anywhere else it arrives as a fresh topic pointing at the earlier record. Nothing is copied when that happens: the pull reads the earlier brief, research, discussion and any concerns still queued against the topic so the new unit's first session opens warm, and the new topic's first conversation reads them again in full and argues with them — while the files themselves stay in the epic that wrote them, untouched. Removing such an item is the one way to say "actually never", and it closes the epic's topic with it.
