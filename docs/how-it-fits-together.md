...
Context is a consumable. A phase's conversation fills the working window with detail the next phase does not need — the whole back-and-forth of a discussion is not something a specification should have to wade through. So each phase concludes by handing off cleanly: it works out what comes next, writes that hand-off down where it will survive a context refresh, and lets you start the next phase in a fresh, uncluttered context. Because the hand-off lives in your files and the durable record rather than in conversation memory, it survives even if the session is compacted or killed between phases. The next phase does not re-interview you; a thin entry skill validates and routes, and the phase picks up the context that was prepared for it — the description, the discovery notes, an epic topic's [brief](discovery.md) — as it starts work.

For an epic there is no single "next phase," so the hand-off returns you to the epic's dashboard to choose your next move. That dashboard is rendered from one computed snapshot, organised into the three stages of [the work's life](work-types.md) — Discovery, Definition, Delivery — with each topic shown under the phase it currently occupies — sorted within each phase by the epic's build order, the suggested sequence the menu's `o/order` action re-derives on request — and markers noting which move is recommended next and which are blocked by unfinished dependencies. Queued [experiments](experiments.md) lead the menu: a topic with live experiments gets a row of its own, ranked above every other recommendation because their verdicts are what unblock a waiting conversation, and the row retires when no live experiment remains — which one to work is settled inside the laboratory. Before it draws, the navigation layer quietly runs its housekeeping: the self-healing analysis that re-reads completed work and can surface topics you had not thought to add.
