...
- **The [engine](engine.md)** — a Node CLI plus in-process library owning everything fully determined by data: manifests, transitions, transactions, derivations, and rendering. Anything computable is computed here, in code, and consumed by Claude — never re-derived in prose.
...
**State discovery runs through a gateway script**, not ad-hoc shell. The skill file embeds `` !`node .claude/skills/workflow-start/scripts/gateway.cjs` `` — the `!` prefix makes Claude Code execute the script during skill load and splice its output into the prompt. The `view` snapshot then arrives in three demarcated sections:
...
**New work routes into [discovery](discovery.md).** A menu pick pre-seeds the work type (`f` → feature); `s` pre-seeds nothing. Discovery shapes the work in conversation, and at the confirm trigger one engine transaction creates the work unit: manifest, imports, seeds, session log, commit. Until that moment nothing exists on disk.
...
Every artifact is markdown in your repo, versioned with your code. Commits happen at natural breaks with conventional messages (`discovery(venue-ordering): exploration notes — session-001`), so `git log` doubles as the workflow's journal — and the context-refresh recovery protocol in every skill leans on exactly that: re-read the skill, read the files, check `git log`, announce your position. Files on disk and git history are authoritative; recollection is not.
