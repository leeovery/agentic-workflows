...
The knowledge base indexes the thinking artifacts: research, discussion, investigation, and specification. Alongside those it keeps a little early-stage context so new work starts warm — the reference material you attached when a piece of work began, the original captured note it grew out of, an epic's running discovery record, and the product roadmap's own session logs and imports. A project that started life before the workflows can also carry a **baseline** — the docs produced by the brownfield assessment interview, indexed as low-confidence reference so what the code shows and what you said about it surface in every later phase without ever posing as a decision of record.
