...
Nothing asks whether to use the mod — it is part of the workflows. Claude Code loads it only where its function hooks, which are early access, are switched on, so every `/workflow-start` writes their switch, `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS`, into the `env` block of the project's `.claude/settings.json` where it is missing — which turns them on for every plugin in this project's sessions. Claude Code reads settings only when it starts, so the start that writes the flag ends with an instruction to restart; the next session has the buttons. The workflows never take the flag out, since other plugins may rely on it.
