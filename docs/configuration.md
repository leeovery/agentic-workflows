...
On a project whose code predates the workflows, your first working session offers a one-time **baseline assessment**: researcher agents read the codebase area by area, then an interview captures the intent, history, and constraints only you can supply — evidence woven into every question, candidate answers to jog memory, and "don't know" always a costless answer. Each finished area lands as a doc the [knowledge base](knowledge-base.md) surfaces in every later thinking phase. The interview pauses and resumes from the start menu, and a completed baseline stays expandable — new areas, or deeper passes over existing ones — from the manage menu. Declining records the answer: the offer never repeats, and the assessment stays available from the start menus whenever you want it. A project that grew up on the workflows is never offered one — the first working session reads the repository's history, records that verdict once, and never puts the question to you — its record is real.

## Session labels

If you work inside tmux, your first working session in a project asks once whether the workflows may rename your tmux session to show where you are — `myproject · payments · discussion · auth-flow` inside a phase, `myproject · payments` at the work unit's menu, `myproject · roadmap` on the roadmap — and put the original name back at the start menu and when the session ends; the label is brought back when you resume the session. You answer once per project. Outside tmux nothing is renamed and the question is never asked.

Whatever you answer, the workflows keep a small session-end hook in the project's Claude settings, which tidies up after a finished session — clearing the markers that tell other sessions a topic is in use, noting where a conversation that ran the workflows keeps its transcript, and restoring your tmux name if you opted in. What the workflows keep for a conversation — where its label was, the buttons it was showing, how each answer sent from a button reads in the transcript — lasts as long as Claude Code keeps that conversation to resume, and goes at the next `/workflow-start` once Claude Code has deleted it. Opting in adds a second hook that puts the label back when you resume a session. You may see them appear in `.claude/settings.json` after your first start.

## Buttons above the prompt

The workflows come with a Claude Code mod that shows their menus as buttons above the prompt, instead of printing each menu into the transcript. The buttons stay put while the transcript scrolls. A click on a row picks it, putting its answer in the prompt box; a second click on that row, or Enter, sends it, and Claude Code shows the sent answer as the plugin's message. You can also type an answer. While any screen but the terminal is attached, such as Remote Control, menus stay text, and a Remote Control screen that attaches after a menu was drawn on the terminal does not get that menu. Pressing Esc before Claude has run a tool in reply to your answer brings its menu back; once Claude has run one, Esc leaves no buttons until the next menu, and so does running `/clear`. While Claude works on something you did not ask for, such as a background agent's report, the buttons stay; a second click then waits for Claude to finish, marked as queued, and sends only if the same menu is still on screen — if the menu changed, it is not sent and the new menu says so. An answer you picked for a menu that has gone is taken out of the prompt box, unless you have edited it. Leave a conversation with a menu on screen and resume it later, and its buttons come back, as long as nothing has happened in it since.

In a workflow session — a conversation that runs the workflows — the mod also makes sure what the workflow shows reaches the screen as written: it turns off Claude Code's one-line summaries of Claude's thinking and its reminder to say what Claude is doing, and puts your own settings for both back when the conversation ends. Your other conversations in the project keep Claude Code's defaults and your own settings, though every session in it gets Claude Code's `SendUserMessage` tool, held back until something asks for it.
