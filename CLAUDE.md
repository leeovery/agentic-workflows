...
**Baseline skill** (`workflow-baseline`): model-only, project-level, outside the pipeline — the brownfield assessment. Reached only from `workflow-start` (the one-time boot offer at Step 0.4 when boot reports `baseline: none` and the project carries pre-existing code; the status-keyed `a/baseline` menu rows; manage). Scope-areas confirm → one `workflow-baseline-researcher` agent per area (dossiers of observed structure + question candidates, never fabricated rationale) → per-area agendas → an AskUserQuestion interview recorded to the agenda ledger every round → per-area docs at `.workflows/.baseline/{area}.md` weaving **observed / stated (from the interview) / open** layers, indexed as they land (KB phase `baseline`, confidence low, reserved pseudo work-unit `baseline`, advisory never record — `knowledge-usage.md` carries the doctrine line). Durable state on the project manifest (`baseline.status`: `in-progress`/`completed`/`skipped`; per-area statuses under `baseline.areas`); dossiers + agendas are committed content under `.workflows/.baseline/.state/` (never indexed). Pausable/resumable; expand reuses the same scoping flow. Displays are the `baseline-*` render-surface family (`projections/baseline.cjs`). Discovery's opener reads `overview.md` in full when it exists; the other docs surface per-topic through the KB.
