...
**Shared references** (`skills/workflow-shared/references/`): loaded by multiple skills across phases. Define protocols and checks that apply uniformly regardless of the calling skill — casing conventions, compliance self-check, topic discovery and dispatch, analysis approval gates, convergence analysis, background agent surfacing, natural break detection, and more; the directory is the authoritative list. Review findings surface in **lanes**: classified by the move owed (never severity) into the protocol's three presentation shapes — batch-apply (shown in full, applied together), walk (one at a time, a genuine choice each), batch-route (sent to the owning topic) — with each caller's `Lanes` declaration owning its lane names, walked-lane heading, and resolutions; a maturity-graded sufficiency bar applied at the agent — early findings are fuel, settled ones face the consuming-phase test; sub-bar findings land in the report as Observations, never raised — the batch fixed at announce time, and promotion only ever toward the walk, never away from it.
