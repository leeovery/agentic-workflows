...
**Epic soft gates**: Forward navigation between epic phases warns via `render epic-soft-gate` (empty on pass). The discovery-side rows count unfinished upstream items; the planning and implementation rows read the build order and name the topics sitting ahead of the selection that are still unplanned/unbuilt. Informational, not blocking — system recovers via re-analysis if user proceeds early. Specification entry is the deliberate exception: it hard-blocks on an unsettled discussion record (see Workflow Phases #6) — a spec cannot be reliably built from an in-flight record.
