...
**Processing skills** (`workflow-*-process`): model-invocable. Assume pipeline context — work_type set, prior phases complete, artifacts in expected locations. Includes `workflow-discovery` for the curatorial discovery-map session that opens every epic.
