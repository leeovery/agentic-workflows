...
6. **Specification** (`workflow-specification-process`): Validate and refine into standalone spec. A spec declares two input types: **sources** (discussions extracted wholesale, gated by `incorporated`) and **consult references** (a sibling discussion read narrowly for an owed correction — cite the hand-off slice, don't extract; gated by `addressed`). Consult references are declared at grouping time (consolidation analysis + a KB advisory query surfaces candidates), carried into the handoff, tracked in the manifest under `specification.{topic}.consult_references.{name}.status`, and block completion until addressed. Source incoherence is met in-flow at construction (`resolve-source-incoherence.md`, loaded from `spec-construction.md`): spec makes decisions clear, never makes them — timeline-resolvable supersession extracts silently; a repair of record lands in the owning source doc (in its own idiom — a discussion's timeline entry — reindexed, `sources stale --except` flagging other extractions) with a note at the next chunk gate; a decision owed always stops (overrides `auto`); a genuine gap routes back to the owning source (epic: triage landing; single-topic types: a direct `topic reopen`) and the spec pauses in-progress — an incorporated row reconciles at re-entry (`session-setup.md`), a pending row re-extracts the updated document. The grouping analysis records advisory `**Tension**` lines construction raises when the relevant chunk arrives. `topic complete` enforces the source gate engine-side (no completion over `pending`/`stale` rows).
