...
**Presence**: per-topic session heartbeat, cache-resident (`.workflows/.cache/{wu}/{phase}/{topic}/presence`, gitignored) — awareness, never mutual exclusion. Session loops `presence beat` each turn; `scan` answers two verdicts per row — `held` (owning pid + start time verified alive) and `live` (held and beaten within 900s). Consumers: the epic view strikes through held topics, skips them as recommendations, and confirm-gates entering one; analysis dispatch defers stale-cache analyses while a peer session is live; the conclude sweep leaves a held row's dirt alone. The SessionEnd hook runs `presence cleanup` for the exits that keep the process alive.
