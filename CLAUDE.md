...
**Epic topic cancellation**: Cancel/reactivate individual topics via workflow-continue-epic menu (`a`/`cancel`, `e`/`reactivate`). Cancellation sets item's phase status to `cancelled` and stashes prior status in `previous_status`. Cancelling a discussion or investigation a live specification sources collapses that spec: the engine refuses the bare cancel naming the spec(s); `topic cancel … --cascade` cancels source and spec(s) in one transaction after the menu's confirm. Cancelled items remain visible in state display but excluded from phase aggregation (`phaseStatus`), gating flags, next-phase-ready logic, and discussion/spec entry discovery. Reactivation restores `previous_status` and deletes the field. Epic-only — other work types use work-unit-level cancellation since topic = work unit.

**Epic soft gates**: Forward navigation between epic phases warns via `render epic-soft-gate` (empty on pass). The research/discussion/specification rows count unfinished upstream items; the planning and implementation rows read the build order and name the topics sitting ahead of the selection that are still unplanned/unbuilt. Informational, not blocking — system recovers via re-analysis if user proceeds early. Specification entry is the deliberate exception: it hard-blocks on an unsettled discussion record (see Workflow Phases #6) — a spec cannot be reliably built from an in-flight record.

**Staleness hops**: When a phase item's upstream input moves — at `topic reopen`, a triage landing, or a spec-side `sources stale` (a discussion document amended in place during specification, no lifecycle transition), never the later re-completion — the engine flags the downstream neighbour one hop away (`transitions.cjs flagDownstream`): the next pipeline phase's same-named `completed` item gets `reconcile_needed = <upstream phase>` (terminal states skipped, an existing flag never clobbered); a discussion's or investigation's downstream is every spec item naming it in `sources` (reverse join), whose `incorporated` rows also flip to `stale` until the spec's own reconciliation re-incorporates them (`stale` blocks spec sign-off alongside `pending`). Entry skills surface and clear the flag via `workflow-shared/references/reconcile-advisory.md` (value-keyed branches); for the linear work types `computeNextPhase` routes back to the earliest flagged phase (`input moved — reconcile`), so the bridge never reads `done` past a live flag; epic surfaces cue flagged items per-item (`· input moved`).
...
**Phase-completion indexing**: Processing skills invoke `knowledge index <path>` at phase completion to add the new artifact. Spec promotion and work-unit cancellation invoke `knowledge remove --work-unit ... [--phase ...] [--topic ...]` to clean up. Pending queue handles transient failures with retry on next `index` call. Corrections to a *completed* unit's specification follow `workflow-shared/references/correcting-historical-artifacts.md` — specification only, the golden record; other completed-unit phase artifacts decay and are never corrected — in-place edit + corrigenda entry, then a single-file `knowledge index <path>` (in-progress units differ: the spec resolution flow — construction or review — edits the owning source document and reindexes it single-file) (replaces that file's chunks; the owning manifest is never touched).
