...
**Session labels**: opt-in tmux rename showing where a session is working (`{original} · {wu} · {phase} · {topic}`, topic collapsed when it equals the wu). `engine session label` is called by every process skill at Step 0 and by discovery at its run step; `engine session cleanup` restores the original name — its own SessionEnd hook on every session skill, alongside (never inside) `presence cleanup` on research/discussion. Opt-in recorded under `session.tmux_labels` in `~/.config/workflows/config.json` (shared with the knowledge key — both sides preserve sibling keys); boot reports it as `tmux_labels`, and workflow-start's Step 0.2 asks once when the value is unset inside tmux. Always a no-op outside tmux, on any tmux error, or without the opt-in; a manual rename is adopted, never clobbered. Original names stash per tmux session under `.workflows/.cache/.session-label/`.
