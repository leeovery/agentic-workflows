...
**Epic soft gates**: Forward navigation between epic phases warns if prerequisite items still in-progress. Informational, not blocking — system recovers via re-analysis if user proceeds early. Specification entry is the deliberate exception: it hard-blocks on an unsettled discussion record (see Workflow Phases #6) — a spec cannot be reliably built from an in-flight record.
