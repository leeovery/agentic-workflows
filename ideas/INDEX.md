...
| # | Idea | Scope |
|---|------|-------|
| 47 | [Position Line](position-line-mod.md) | One line pinned under the prompt — `{wu} · {phase} · {topic} · task N/M` — drawn by the workflow-gates mod via `$.ui.status`, read off the engine's own `session label` and `task start` calls. In-app counterpart to the tmux label; no opt-in. Depends on the mod (`design/function-hook-gates.md`). |
