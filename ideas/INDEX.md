...
| 22 | ~~[Editing Historical Phase Artefacts](editing-historical-phase-artefacts.md)~~ | `engine topic amend` + `knowledge-usage.md` **E** + planning-entry advisory + `spec amended` dashboard cue | ✅ Done — landed narrower than the idea file describes: **completed specifications only**, cross-work-unit, reached via the knowledge base. Specs are the one artifact `compact` never decays, so a wrong claim in one is served forever; research/discussion/investigation are records and decay on their own, so they are never corrected. In-unit corrections keep using reopen (`validate-phase.md` → `spec-completion.md`), which already reindexes. |
