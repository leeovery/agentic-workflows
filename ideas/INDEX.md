...
| 41 | [Coherence Analysis — Conflicting/Stale Decisions Across an Epic's Discussions](discussion-coherence-analysis.md) | Third sibling analysis next to `research-analysis`/`discovery-gap-analysis`: full-corpus read of an epic's completed discussions → unacknowledged conflicts, stale references, owned ambiguities → findings gate → `topic triage` reopens the yielding discussion. Engine cache kind (`phases.discovery.coherence_analysis_cache`, minInputs=2), `dismissed_findings` fingerprints, spec-entry soft advisory. Stack of 2 (engine → prose) + prose-test case PR. |
