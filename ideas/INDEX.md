...
| 33 | [Knowledge Chunk Timestamp Uses Index Time, Not Document Date](knowledge-chunk-timestamp-uses-index-time.md) | Knowledge base — `src/knowledge/index.js` `indexSingleFile` stamps every chunk with `Date.now()` instead of the source doc's date, so bulk/fresh indexing (install, reindex, migration) makes all legacy docs read as "today" in query headers — breaking provenance & recency. Fix: derive `timestamp` from source mtime (in scope as `absSource`), optionally frontmatter-date → mtime → now. |
