...
- **PR 3** — kernel manifest IO (atomic write), `domain/map.cjs` transitions + `mapState` (decision-ready: counts, `all_decided`, `unresolved`), `engine map add/set` (JSON responses carry derived state — no follow-up read), discussion-map projection + a `workflow-discussion-process` adapter (`map` verb: DATA incl. `review_cycles` + DISPLAY), `discussion-session.md` re-pointed (record via engine; display via adapter; convergence from DATA; conclude-anyway sets `deferred`), the file template drops its Discussion Map section, migration 045 parses in-flight discussions' markdown maps into the manifest (defensive, idempotent) + test.
- **PR 4** — the standalone transactions: `engine topic cancel/reactivate` (the five-step epic flows), `engine inbox archive` (+ restore/delete), the scoped session-commit helper. Epic/menu/.md call sites re-pointed.
