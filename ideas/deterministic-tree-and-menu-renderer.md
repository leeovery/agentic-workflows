...
**Wave 4 validated against the prose original (2026-07-13):** all 25 diff hunks across the six skills audited as pure re-points (structure/gates/displays byte-untouched); twin-fixture equivalence proven — old prose command sequences vs new engine commands produce byte-identical manifests, identical commits (init→conclude incl. KB-fail tolerance; spec+planning register chains). Schema-enforcement fix landed after a live session used `topic complete` to bypass the manifest CLI's refusal: status vocabulary extracted to workflow-shared/scripts/manifest-schema.cjs, required by BOTH enforcers; lifecycle commands refuse the discovery phase (map items carry no status). Observed pre-existing looseness, unchanged: the KB store (tracked files) has no owned commit point — mutations linger until a broad commit; candidate for a later wave.

**PR 14 (#405) — wave 5 first half, specification (2026-07-13):** the entry's seven-way scenario routing (prose counter-arithmetic in check-prerequisites + route-scenario) is now derived in `domain/specification.cjs` and shipped as the DATA `scenario` key; the five display template families become projections (`groupings`, `specs-menu`, `single` ×3 variants, blocked ×2, completed sub-view) with plural-fixture goldens; the adapter gains `view {wu}` + `completed-menu {wu}` while both labelled-dump forms stay byte-identical (untouched 42-test suite + a live sandbox twin-run). Menus route on action keys (`start_spec`/`continue_spec`/`unify`/`reanalyze`/`analyze`/`completed_menu`/`refine_spec`); the Creating/Continuing/Refining verb is data-derived and rides the ACTIONS entry. Process lifecycle → `topic start` (create-or-flip; the `created` flag replaces the empty-vs-proposed branch) and `topic complete` (owns the KB index) incl. promotion's inner chains; `{wu}` commits → `engine commit` verbatim; promotion's dual-work-unit commit stays raw with explicit staging. Three legacy defects corrected, not codified: repeated `└─` sibling glyphs (the whole template family), the duplicated closing sentence in blocked-no-discussions (reviewer fix), single-grouped's placeholder/prose contradiction (resolved per prose: spec name). Stays in the manifest/KB lane: source & consult statuses, gate modes, reopen (topic start refuses completed by design), the judgment-laden groupings reconcile. **Recommended, not built: `topic supersede`** — the two supersession call sites disagree on what `superseded_by` points at ({topic} vs {work_unit}/unified); needs a design decision first. Consult hints for proposed groupings are parsed from the analysis doc adapter-side (the adapter owns file IO) and passed into the domain ring as input.

**PR 15 (#406) — wave 5 second half, planning (2026-07-13):** .md-only, zero engine code — planning has no read gateway (entry always receives a topic) and the format-blind law keeps everything backend-facing in prose. Lifecycle: `init-phase` → `topic start` (crash-rerun now heals; no `created` branch — Step 0 guarantees absence), conclusion → `topic complete` (planning excluded from `INDEXED_ARTIFACTS` — matches the old prose's no-index, and the KB charter). Commits split into two explicit lanes, stated once in a new process-SKILL Hard Rules section: `{wu}`-scoped prescribed sites → `engine commit` verbatim (11 sites incl. the review-criteria files the planning review agents execute — their unscoped Bash grant covers it); out-of-scope-staging sites → raw git with explicit `git add --` (plan init writes `project.defaults.plan_format` into `.workflows/manifest.json`; restart/author/graph/findings stage format task storage). Defect fixed: unquoted `task ~` in plan-construction (zsh `$HOME` expansion). Flagged for follow-up, untouched: `agents/workflow-planning-review-*` self-commit with `planning({topic})` messages vs the criteria files' `planning({work_unit})` — recommend aligning to the spec review agents' no-git-writes contract. Recommended against a `plan init` engine transaction (format name through the CLI boundary + cross-manifest write). Entry reopen stays manifest-lane; `resolve-dependencies` G's external-deps display stays prose (half its content is session-derived). Wave 5 complete.

**Wave 5 live campaign passed (2026-07-14/15), zero findings.** Spec session: entry `view`/`completed-menu` verbs at the right moments, stale-cache delete, KB advisory queries, reconcile on the manifest lane, cache-metadata sets (intentional — the consolidation cache is not a `cache stamp` kind; its checksum is copied from the discovery DATA, no drift risk), `engine commit` verbatim, snapshot re-run; every emitted render (specs-menu, completed sub-view, re-emitted menu, post-analysis groupings) **byte-identical** to the engine sections — the dangling-`└─` fix confirmed live (`├─ Spec:` above `└─ Discussions:`). Planning session (auth-flow feature, spec seeded via engine): 13-commit ledger all correct — raw dual-stage init commit (`.workflows/manifest.json` + wu), draft/approve phase + task-list + complete-phase via `engine commit` verbatim, **6 × per-task raw `git add -- .workflows/auth-flow .tick`** author commits (the format-storage lane followed exactly — no invented flags, unlike the pre-fix wave-4 drill), quoted `'~'` sets live, `author_gate_mode auto` set on the user's `a`. Gate renders match templates throughout.

**PR 16 (#407) — wave 6, review (2026-07-15):** .md-only, zero engine code — same shape as planning (no read gateway; entry untouched entirely). All five completion sites in the actions loop → `topic complete` (checked individually); `init-phase` → `topic start` behind its kept `exists` guard (the process skill is model-invocable; start refuses completed). Commit lanes: wu-scoped prescribed sites → `engine commit` verbatim; **surface-to-inbox → `engine commit --inbox`** (stages only `.workflows/.inbox/` — first prose use of the inbox lane); do-now fixes + remediation task writing → raw git with explicit staging (project files / format storage). Cross-phase reopen (`implementation` → in-progress on send-back) stays manifest-lane by design. Defects fixed: undefined `{scope}` commit placeholder; a format name leaking as an example wu name in the re-open handoff. **Flagged, preserved:** 4 of 5 completion sites set `completed` with no commit before the terminal bridge stop — the manifest change rides uncommitted into the bridge (same class as the KB-store looseness; deferred decision). Also pre-existing: the process allowed-tools `mkdir -p .workflows/.inbox` entry doesn't prefix-match the prescribed `mkdir -p .workflows/.inbox/{category}`. **Live campaign passed (2026-07-15), zero findings** — login-crash bugfix review end-to-end: `topic start` behind the guard, 3 verifier dispatches, `reviewed_tasks` pushes, report commit via engine, **first live `engine commit --inbox`** (staged exactly the one surfaced idea file), Request-Changes verdict → synthesis declined → `topic complete` → bridge pipeline completion (wu `completed` + commit). Gate renders match templates byte-for-byte. The completion-commit gap (ledger #1) self-healed in this path — the bridge's pipeline-completion commit picked up the status flip; it only bites for epic topics where the bridge doesn't complete the pipeline.

**PR 17 (#408) — wave 7 first PR, `engine workunit create` (2026-07-15):** the work-type commit as one transaction — new `workunit` noun (the work unit as domain object; future home for start-wave lifecycle ops). Creates the manifest byte-identically to `manifest.cjs init` (equality-tested; `RESERVED_WORK_UNIT_NAMES` lifted to the shared schema), lands imports (normalisation/dedupe contract absorbed from prose), moves seeds with folder-derived provenance, installs the model-authored session log verbatim, sets the epic marker, commits `{wu}`+inbox scoped. Fail-fast `missing_imports` powers the re-prompt loop from the error response. `import-files.md`/`land-seed.md` retired (single-caller); confirm-trigger 123→83. Supporting: `domain/kb.cjs` (one KB door, extracted verbatim from transitions), multi-pathspec `commitScoped` (back-compatible). Deliberate resolutions flagged on the PR: re-import contract settled suffix-always (old prose self-contradicted); inbox staged only when seeds moved. 19 new tests (501 total). Smoke-run verified live in a scratch fixture.

**PR 18 (#409) — wave 7 second PR, discovery-map operations (2026-07-16):** six Tier-2 verbs land on `discovery-map` per the reserved taxonomy slot — `edit`/`remove`/`rename`/`reroute`/`handle`/`reactivate`, decision-ready JSON, no in-op commit (discussion-map precedent; sessions commit via `engine commit {wu}` with verbatim messages). Lifecycle gates enforced in the engine by consuming `computeTopicLifecycle` from workflow-shared (the epic dashboard's join — one computation, two consumers; the schema-bypass lesson applied to the map). **Latent bug fixed:** the prose rename read-and-rewrote only four known fields, dropping `order`/`brief_path`/`brief_incorporated`/`legacy_split_state`/accumulated `source`; the engine rename is a true key swap, everything preserved, position kept (test-pinned, smoke-verified). `VALID_ROUTINGS` unified into the shared schema (manifest.cjs inline literal removed). map-operations.md 592→540. 24 new tests (525 total).

**Wave 7 live campaign passed (2026-07-16), zero findings.** Session 1 (new-work creation): inbox pickup promoted two ideas → discovery shaped an epic → `engine workunit create` landed manifest + both seeds (pure git renames out of the inbox) + the normalised import + the model-authored session log in ONE commit, no warnings; topic synthesis then built a 3-topic map (unmigrated prose, raw commits as expected). Session 2 (map ops): all six `discovery-map` verbs exercised behind their gates — rename preserved summary/description/`brief_path`/`order` live (the fixed latent bug, confirmed in the wild), remove → dismissed list, handled round-trip clean, per-item `engine commit` messages verbatim, zero refusals. **Feedback for the session-loop PR:** (a) the hand-drawn "Synthesised Discovery Map" render reads badly (ragged `[routing:…]` tags, uneven wrapping) — Lee flagged it live; design a proper projection, never reproduce that layout; (b) synthesis-created map items carry a redundant `status: in-progress` field (map items are status-less by design; lifecycle is computed) — trace where `create-discovery-topic` stamps it and whether it belongs.

**PR 19 (#411) — wave 8, discovery session loop (2026-07-16):** the last discovery-skill migration. `discovery-map add` creates status-less map items (replacing `pull dismissed` + `create-discovery-topic`; `--force-dismissed` carries the gate-confirmed re-add; `map_total` rides the response). The flagged "Synthesised Discovery Map" render and the opener anchor become one projection family (`map-view {wu} [--proposed-file]`) — lifecycle labels as tags, kernel-wrapped summaries, ragged layout structurally impossible; `discoveryLifecycleLabel` lifted to conventions, shared with the epic dashboard (epic goldens byte-unchanged). Synthesis DATA adds pre-gate `exists_on_map`/`matches_dismissed`/`legal_name` flags — collisions resolve before the confirm gate. All session-loop commits → `engine commit` verbatim. **Status-field trace:** `cmdCreateDiscoveryTopic` stamps the dead `status: in-progress`; six call sites remain outside the fence → ledger item. 23 new tests (548 total); renders smoke-verified on real sandbox state. **Wave 8 live drive (2026-07-17): write path clean, two fixes landed on the PR.** Session-002 on output-classification: staged proposed-topics.json → synthesis render byte-faithful → `add` with full summary+description → brief + `brief_path` → marker delete + synthesise commit (correctly scoped) → KB index → trailing finalise call no-opped on a clean tree. Bonus: main's 046 migration ran + committed via `engine commit --workflows`, validating the 047 renumber live. Fixes: (1) map trees attach to their headers (Lee caught the orphaned `├─` head — dashboard convention; goldens pin it; repo-wide sweep found no other detached trees; attachment rule queued for CONVENTIONS in the hardening PR); (2) the resume branch soft-matched a continuing epic (continuity-load on a populated map), skipping the opener map anchor — empty-map requirement hoisted into the branch heading with an explicit fall-through pointer. Also observed: a `> *Output…*` directive line leaked into an emitted block during the migration summary — the reasoning-surface-thinning class, noted for platform deepening.

**PR 23 (#415) — reasoning-surface thinning (2026-07-17):** the deferred wave lands. Every startup printout audited field-by-field against its prose consumers (the consumption map), then thinned to the decision-ready minimum: epic index ~21 lines/epic → 4 (+8 scoped); spec 17→2; bridge 11→3; start 44→29. Nine byte-stability suites deliberately rewritten to pin the new bytes (two had silently fallen out of npm test — re-wired; count rose to 656). Latent gaps closed: inbox titles, closed-set listings, `routing=` on map rows — all consumed by prose the old dumps never satisfied. `all_done` derives the bridge's five-check epic-done test in code (all-cancelled review = false, pinned). **Review at Lee's heightened bar: four independent adversarial subagents (blind consumption-map verifier, test-rewrite auditor, behavioural-equivalence runner on real sandbox state, estate vocabulary sweeper) — all CLEAN**; their four findings (wording, pseudocode symbols, stale comment, dotted labels) applied. Views byte-identical throughout.

**PR 24 (#416) — island absorption (2026-07-17):** one manifest IO implementation (`workflow-shared/scripts/manifest-io.cjs`, both writers consume it) and the lock gap closed — every engine transaction now holds the CLI's lock across its read-modify-write (full coverage table; locks never nested; KB/commits after release, `.lock` never stageable; live-verified block-and-wait). Project-manifest clobber window in create/pivot/absorb closed (fresh read under lock). **Serious catch: the manifest CLI suite was red at baseline** — PR 22's schema tightening broke its discovery pins and `set -e` silently skipped ~80 tail assertions; repaired (268/268) and the suite added to the standing gate list. KB lanes classified: 3 mechanical stragglers absorbed (cache stamp indexes its caches; discussion conclusion joins topic complete); recommended-not-built: `discovery-session close`, `workunit promote`. 673 tests. **Open design discussion (Lee, in progress): full surface absorption** — manifest CLI retiring into engine field verbs (`engine set {wu} {phase} {topic} key=value` under grammar rule 5), knowledge CLI reduced to the human setup/rebuild tool with in-process engine calls; churn ruled a non-cost by Lee; open points: per-skill permission granularity, setup's home. PR 24 is a stepping stone under either ruling.

**Platform structure RULED (Lee, 2026-07-18):** knowledge skill + script stay entirely as-is (separate subsystem — a retrieval database; human setup/rebuild; advisory lane). **Manifest absorbs into the engine as `engine manifest <cmd> <dotpath> …`** — dot-path KEPT on the merits (document addressing, self-disambiguating two-arg path+field; the positional alternative needs resolver machinery for variable-depth ambiguity), not inertia. Deliberate de-legacying: mutations adopt the engine JSON response; reads keep bare-value stdout (substitution ergonomics — split contract on purpose); optional `field=value` batching on set; retired commands (init/init-phase/create-discovery-topic) do not come along. Code centralisation: `workflow-shared/scripts/*` (schema, io, discovery-utils) dissolve into engine kernel; manifest.cjs path becomes a logic-free grammar shim (or retires once prose re-points — builder surveys); permissions via sub-verb prefix `Bash(…engine.cjs manifest)`, fallback cost mild. **Parked (post-launch): conversational KB setup** — boot detects not-ready, Claude presents provider options in chat, applies via parameterised non-interactive setup; API keys must NOT pass through chat (terminal/env handoff step stays).

**Knowledge separation CONFIRMED (Lee, 2026-07-18):** knowledge stays its own skill — the asymmetry is principled, not residue. Manifest absorption was justified by sibling-writer drift risk (same files, two writers); knowledge is a standalone retrieval subsystem (verified: zero knowledge→engine references; engine is an arms-length subprocess client, warn-don't-block, no shared code or files). Rule of thumb recorded: the engine owns workflow state; knowledge serves retrieval. **Engine SKILL restructure ruled:** it is a fallback documentation site, not a workflow skill (prose instructs exact engine calls inline; the skill is consulted to understand, not to obey) — SKILL.md becomes compact orientation + references/ for grammar, per-noun commands (incl. the incoming manifest section), and the output-section contract. Style bar: terse over verbose, zero repetition, every sentence earns its keep. To be done on the PR 25 branch at review.

**PR 25 (#417) — manifest absorption (2026-07-18):** the ruled structure lands. `engine manifest <cmd> <dotpath>` (bare-read/JSON-mutation split, `=`-batching under one lock, retired verbs refusing with targeted successor pointers); `workflow-manifest/` deleted, `workflow-shared/scripts/` empty (modules → engine kernel/domain); 379 call sites re-pointed; allowed-tools scoped via the `…engine.cjs manifest)` prefix for 10 field-only skills. **Four-reviewer fleet:** re-point verifier CLEAN (live-executed transforms byte-identical; 33-case read battery vs the resurrected CLI), equivalence runner CLEAN (old CLI leaked locks on 4 failure classes; engine leaks zero), sweeper CLEAN, **auditor NOT CLEAN → all applied** (7 tautological assertions made real — the shape test now pins fixture↔engine byte-equivalence; batched-set lock hold dynamically tested; refusal redirects restored). **Systemic find closed: 19 orphaned suites** incl. the contract suite itself never gated — 470 tests rescued into npm test (1,162 total), new `test:cli` gate for shell suites, the rotten map-operations suite deleted (pinned retired sequences + the dead status field). `workunit create` tightened: `--no-session-log` explicit. **Engine SKILL restructured as a documentation site** (37-line orientation + references/{commands,library-and-gateway}.md) per the ruling.

**KB init design RULED (Lee, 2026-07-18):** conversational initialisation lands pre-launch (defer nothing). System-config-detected path is primary (confirm in chat → `knowledge setup --from-system` → session continues, no restart); fresh-machine menu orders AI retrieval first (OpenAI recommended, compatible second), **keyword-only explicitly last as backstop**; terminal shrinks to the one irreducible case — a first-time secret not in env — minimised via new `knowledge setup --key-only` (10-second detour, session resumes, no restart). Global reconfiguration stays interactive by design (system-wide act, wizard's job). Keys never through chat; no `--key` flag exists. Wizard rewrites old-schema system configs clean (decay_months fossil class). Also ruled: nothing defers (`workunit promote` + `discovery-session close` building in PR 27); Opus is the fidelity baseline; releasing is mint's (docs accuracy is our whole obligation).

**PR 26 (#418) + PR 27 (#419) shipped (2026-07-18):** #418 — gateway rename (10 adapters + 9 suites, rename-purity proven, every head-insert verified live), docs accuracy pass (CLAUDE.md pipelines now start at Discovery for all five types; Test Gates section; README + knowledge SKILL gaps; the false store-outside-git claim gone), chrome normalisation (5 files, independently re-swept to zero violations); two-reviewer fleet CLEAN. #419 — `workunit promote` (validate-first, tri-pathspec commit incl. the project manifest the prose never committed; array-shaped-sources refusal from review) + `discovery-session close` (marker clear + index + commit, fixing the commit-then-index stranding); review also closed the same registration-staging gap at `workunit create` (pinned). Rebased across the rename (3 conflicts, resolved toward the rewrites). Stack: 29 PRs, #380+#383→#419. One flaky timing-test occurrence logged for fleet E. **Build phase COMPLETE except the KB conversational-init PR.**

## Endgame (revised 2026-07-17, Lee's call)
...
**PR 21 (#413) — closing transactions (2026-07-17):** `topic supersede` (schema-gated, `superseded_by`=topic per ruling, no-commit batch semantics, handoff prose corrected), `workunit pivot` (dual-manifest flip incl. the fixed registration gap; engine-owned commit fixing the no-commit gap; `reindex-work-unit.md` retired — ledger #6 resolved), `workunit absorb` (validate-first transaction closing the ~12-command crash window; pristine-on-refusal test-proven; timestamps preserved; tri-pathspec commit). Seeds joined the re-index walk (ledger #7 resolved). Six obvious-gap fixes flagged under the new standing rule, incl. two absorb data-loss guards (on-disk file collision; shape-invalid entries refuse instead of skip-then-delete). Recommended: discovery session logs join the walk (hardening). 27 new tests (586); absorb + pivot independently smoke-verified.

**PR 22 (#414) — hardening sweep (2026-07-17): the deferred ledger is closed.** All eight items landed: review completion commits (4 sites); KB setup-commit handling (**Lee reversed the auto-init post-review** — the `not-ready` hard stop at `knowledge setup` is deliberate, auto keyword-only breeds permanently-unconfigured projects; what ships is the actual ask: the post-setup restart's boot commits the untracked store as `chore(knowledge): initialise store`, real-CLI-verified) + the every-engine-commit-stages-.knowledge rule (`commitScopedWithKb`, exists-guarded); planning review agents → no-git-writes (orchestrator owns tracking-file commits, `{work_unit}` message form); CONVENTIONS List-Display fix + explicit tree-attachment rule; `create-discovery-topic` fully retired (wrapper → `discovery-map add` + `topic start`; command deleted; `VALID_PHASE_STATUSES.discovery` = empty-list-never-removed with the permissive-path rationale; migration 048 strips dead status fields, 10 tests); allowed-tools inbox glob; discovery session logs join the re-index walk. Build note: the background builder hit an API-credits wall after six items; the rest was finished inline. 593 tests, 48 migration files, typecheck clean.

**PR 28 (#420) — KB conversational init (2026-07-18): the build phase is COMPLETE.** Non-interactive `knowledge setup` forms (`--from-system`, `--keyword-only`, `--provider openai|openai-compatible`, `--key-only`); boot's `not-ready` gains `system_config: {status, provider, model}` (config.json only, never credentials); `workflow-start` Step 0.3 → `references/knowledge-gate.md` conversational gate per Lee's rulings — AI options first with keyword-only as the LAST backstop, `d` per-project deviation never touches the global config, global reconfigure stays wizard-only, the one terminal detour is first-time key entry via new `--key-only`. Deliberately no `--key` flag (argv → shell history/process listings). Two review fleets: the gate reviewer found 4 routing defects (headline: `d` routed into the provider forms which rewrite the GLOBAL system config — confined to keyword-only/wizard); the key-leak adversary found a genuine BLOCKER — upstream HTTP error bodies are untrusted, and a header-reflecting endpoint put the full Authorization bearer on chat-bound stdout via non-auth statuses (canary-proven on 400/404/422). Fixed at the source: Bearer + `sk-` redaction and a 300-char cap on all error detail, canary-verified clean at every status with the adversary's own scanner. Also fixed: `askSecret`'s readline keypress echo survived `rl.pause()`, co-echoing typed key chars in plaintext beside the masking stars (pty byte-proven, now pure stars); `detectSystemConfig` no longer propagates JSON.parse content snippets; gate prose gained a pasted-key handler (never echo, advise rotation, redirect to `--key-only`). 1,205 node tests + CLI/migrations/typecheck green. **Stack is 30 PRs (#380, #383–#409, #411–#420). Next: the five-fleet deep review campaign, then Lee's stack review, `stack merge`, launch with fix-forward.**

**Five-fleet deep review campaign (2026-07-18): system certified, four fix PRs (#421–#424).** Lee's end-of-build ultracode-scale review, run as five parallel adversarial fleets against the stack head. Verdicts: **zero BLOCKERs across all five.** A (design conformance): 6/9 items fully conform; deviations were the lib-door bypass (5 direct domain requires), a stale README setup section, and the "zero knowledge→engine references" record claim being overstated (the CLI resolved engine.cjs at module load). B (prose↔engine): 458/458 command invocations shape-clean; 2 HIGHs were allowed-tools declaration gaps (four continue-* skills couldn't run their workunit transactions; discovery lacked the knowledge declaration). C (drives): full PASS — ~240 invocations across all five work types + pivot/absorb/promote/inbox; 17 refusal scenarios all pristine; 55 engine-owned commits all correct; docs-level findings only. D (adversarial): core integrity held (zero lost updates under concurrent hammering, fully-before-or-after at every SIGKILL, traversal contained); real gaps: stale-lock break TOCTOU, array-root false ok:true, 200k-token indexer crash with stack-trace leak (root cause: orama replaceDiacritics spreads one arg per char into String.fromCharCode — V8 arg limit, not recursion), ANSI passthrough in query/dashboards. E (test integrity): "the green estate genuinely certifies the system" — zero tautologies, zero orphaned suites, 48↔48 migration bijection, all seven charter guarantees genuinely tested; flake did not reproduce in 14 runs but the estate sweep pinned the likely cause ('exit' fires before stdio flush in three lock tests).

**Fix PRs** (stacked #421→#424 on #420, four parallel builders, zero rebase conflicts, gates green at head — 1220 node tests, 49 migration suites, bundle byte-reproduces): #421 engine integrity — O_EXCL `.breaking`-guard lock break (the builder proved my prescribed rename-based break was itself racy — a stale-stat loser can rename the re-acquired holder's fresh lock away — and replaced it with a guarded re-check protocol that zeroes Fleet D's attack), root/container shape validation (no false ok:true, domain diagnostics), lock-timeout remedy hint, orphan-tmp sweep + migration 049 (nested `.workflows/.gitignore`; fixes the never-stageable root gitignore from 010/011). #422 knowledge robustness — 2048-char token-run splitting (bytes kept), top-level no-stack-trace catch, C0 stripping at query output, lazy engine resolution (keyless commands now engine-independent — this also makes the standalone claim true at load time; the record stands corrected: knowledge→engine is subprocess-at-use-time for manifest reads, by design), similarity test de-vacuoused. #423 prose/docs — both allowed-tools HIGHs, vestigial-declaration removal (bridge's kept — builder disproved the fleet's claim via load-closure trace), confirm-trigger "never indexed" drift, README conversational-gate rewrite (the docs-accuracy release obligation), commands.md exit-code/absorb/grammar corrections. #424 rings/tests — 'exit'→'close' ×3, lib `engine.discovery` namespace closing the five-file bypass, discussion-map transactions into domain (engine.cjs no longer touches kernel manifest IO), render assertion strengthened.

**Deliberately unfixed — Lee's call:** (1) long topic names overflow tree width (fix implies a name-length limit — UX contract change); (2) `~` sentinel means null in `task complete --next-task` but lands as the literal string via the field surface (consistent in practice, semantically fuzzy). Minor accepted residue: `knowledge compact` silent on no-op (boot wraps it), UTC-midnight date assertions in four tests (fail only across a date boundary). **Stack is 34 PRs. Next: Lee's stack review → `stack merge` → launch with fix-forward.**

**Review round + codification wave (2026-07-19): stack at 41 PRs (#380, #383–#409, #411–#431).** Lee's stack-review tweaks shipped as #425 (versioned banner byline `Agentic Engineering Workflows (v0.5.13)` — mint mirrors it each release via `[release] version_file`/`version_pattern` with the `{version}` placeholder, semantics proven by a real release in a scratch repo; boot announcement to code block; knowledge-gate accuracy + mandatory phrasing), #426 (title-composer label clamp at 40 chars — ruled: display clamping, never a name limit; first attempt in the kernel broke real fixtures, the composer is the layer that knows which part is the name), #427 (bare `~` = null across the field surface — ruled: one sentinel spelling; readers already dual-tolerant). Lee's stale stash (`idea/inception-pr-16-cleanup`) verified byte-redundant with landed work and dropped. Lee then challenged "complete with 4 items listed?!" — ruling: the codification wave IS part of done. #428 workflow-state commits → `engine commit` (per-site sweep; format-storage/code commits stay raw with reasons now explicit), #429 `topic reopen` (ten sites re-pointed, not the six surveyed; conclude-implementation proven a `topic complete` drop-in), #430 `discovery-session open` (disk-derived numbering — no stored counter exists; log-before-marker crash ordering), #431 strict continue-* fallbacks (permissive path had zero legal traffic). With #431 the codification survey is drained: no prose site performs a workflow-state write or lifecycle transition outside the engine. Head gates: 1266 node tests, all suites green. Flake log: one ENOTEMPTY teardown in test-legacy-research-split (first run only, unrelated, clean on rerun).

**API design review (2026-07-19, awaiting Lee's ruling):** macro verdict — the surface is well-designed (uniform noun-verb grammar, symmetric lifecycle families, unwavering response/exit contract, coherent transactions-commit/mutations-don't law). Three fix candidates presented: (1) `discovery-map add` routing → positional (kill the documented rule-2 exception; `reroute` already takes the vocabulary positionally); (2) `handle`/`unhandle` rename dissolving the cross-noun `reactivate` collision (map's reactivate clears `handled`; topic/workunit's restores cancelled); (3) read-side naming debt — `discovery-utils.cjs` is an accidental general library mislabelled as `engine.discovery`, `domain/{epic,workunit}.cjs` collide with `projections/{epic,workunit}.cjs`, `discussion-map.cjs` mixes ops+view where discovery splits them. Ruled out as motion-not-improvement: splitting `fields.cjs`, nouning `boot`/`commit`, dropping `render`.

**API design fixes (2026-07-19, ruled by Lee — all three): stack at 44 PRs (#380, #383–#409, #411–#434).** #432 routing → positional on `discovery-map add` (grammar rule-2 exception deleted, `--routing` extinct repo-wide, no alias; absorb/pivot untouched — they register in-process). #433 map `reactivate` → `unhandle` (clean inverse pairs engine-wide: complete↔reopen, cancel↔reactivate, handle↔unhandle; every surviving `reactivate` verified restore-from-cancelled). #434 read-side reorg, zero behaviour (byte-suites untouched): `discovery-utils` → `reads.cjs` + `derivations.cjs` (reads ← derivations enforced), lib namespace `engine.discovery` → `engine.reads`/`engine.derivations` across the five consumers, `epic.cjs`/`workunit.cjs` → `-detail` names, `projections/discussion.cjs` → `discussion-map.cjs` completing the ops/projection pairing (mapState/subtopicsOf stay ops-side — transactions depend on them). ENOTEMPTY teardown flake in test-legacy-research-split (twice under parallel load) fixed with rm retries riding #434. Head gates: 1266 node, 328+5+40 shell (approx per-suite), 49 migrations, typecheck, byte-suites 358/358 isolated. **The design review is closed: grammar has zero exceptions, no verb collides across nouns, and the read side is modelled as deliberately as the writes.** Next: stack sync via the stack CLI (dry-run first), then Lee's review → `stack merge` → launch fix-forward.

**Final certification round + fix wave (2026-07-19): stack at 51 PRs (#380, #383–#441).** Nine-agent certification at #435's head: all five pipeline walkers (epic/feature/bugfix/quickfix CONTINUOUS; cross-cutting BROKEN at one pre-existing join), change-discipline audit DISCIPLINED (24 commits/107 files, zero undiscussed changes), conventions PASS (no HIGH; CLAUDE.md accurate claim-by-claim), live-drive mostly PASS, prose↔CLI 571/571 valid with extinction sweeps all zero. Key insight: round-2 HIGHs were almost entirely PRE-EXISTING system defects (cc spec-entry never had its branches; revisit-scoping always dead-ended; the promoted-spec source leak; the systemic orphan window) — the review strata keep digging into older rock. Lee's rulings on the five synthesis items: (1) orphan window → surface as *finalising* with a finalise action + terminal-reactivate refusal; (2) provenance tags derive from actual state (superseded visible); (3) declare git add/commit at the deliberate raw sites (breadth accepted, prompts are friction); (4) initialize-plan keeps --workflows (three-scope law over sweep risk); (5) carrier bypass restored (completed research gets read-in-full on the carrier path). Fix wave #442–#447 (recreated after a stack-sync incident: sync ran with builder worktrees still holding the branches, collapsed the wave refs to one tip, GitHub marked the emptied PRs merged; `stack undo --apply` restored every ref exactly and the six PRs were recreated verbatim — originally #436–#441) (six parallel builders, restack clean, 1291 node tests at head): cc spec-entry branches; bridge stored-route-is-authoritative (§F never reconstructs invocations); scoping continue/restart gate; engine truth (finalising surfaces + honest tags + epic adapter hardened + label/plural/body/banner — incl. the builder-caught complete-from-cancelled deadlock fix); ten-fix prose sweep (incl. commit-scope correction on quick-fix promotion); allowed-tools closure alignment (bridge/continue-epic knowledge declarations now genuinely dead — KB ops live inside cancel/reactivate transactions). Stack synced (48 open PRs; one retry after builder worktrees blocked branch rewrites — remove worktrees before sync). **Next: combined 9-agent round — 4 targeted verification on the wave (engine-truth adversary, both mechanical sweeps, touched-surface drive) + 5 pre-existing-defect hunters (agent briefs, output-format adapters, shared refs, process logic ×2, loop termination) per Lee's ruling to keep digging for pre-existing errors.** Open observation: empty-state `v` branch dangle (right fix arguable — backbone re-check vs menu re-render).

**Round-3 hunt + fix waves (2026-07-19): stack at 55 PRs (#380, #383–#435, #442–#451).** Lee ruled the pre-existing-defect hunt in ("im also keen to find and fix pre-existing prose / logic errors too"): 9-agent round — 5 hunters into under-swept strata (agent briefs, format adapters, shared refs, process logic ×2, loops) + 3 verification + adversary re-run. Haul: ~65 findings, 1 BLOCKER (edits-only discovery sessions couldn't exit), ~10 HIGH — headline pre-existing defects: the spec `incorporated` gate never gated (listed after topic complete), in-flight agent detection structurally blind (status files only exist post-completion), auto-mode authoring wrote zero tasks, the fix-attempt cap self-destructed on resume, spec_commit re-baselined before reconciliation, triage-landing stubbed over completed artifacts (2 witnesses), the tree's one autonomous livelock (author count-mismatch retry), the two-witness agent-allowlist HIGHs (grapher/task-writer couldn't execute tick/linear adapters). Loops audit PROVED the termination story sound (consent-paired resets, STOP on every back-edge). Adversary re-run on the finalising work: NOT SOUND — the reactivate refusal deadlocked the documented reopen path (its rationale obsoleted by the same diff's surfacing — ruling reversed, refusal removed), finalising rendered false state over reopened phases (now requires zero in-progress + re-points next_phase), three tag-honesty siblings. Fix waves #448–#451 (four builders + adversary scope-add mid-build via SendMessage; 47+11 items, zero skips; one rebase conflict merged both fixes) + Lee's strict-conventions directive → dedicated gate with edit rights: PASS after 4 fixes (illegal @elseif, spacing, link bolding, routing arrow) over 86 files. Head gates: 1304 node tests, all suites green. Synthesis record: scratchpad/round3/SYNTHESIS.md. Next: worktrees removed → stack sync → final mechanical sweep at the finished head → Lee's stack review → merge → launch fix-forward.

**r4 review round + menu codification (2026-07-19 cont.): stack at 57 PRs (#380, #383–#435, #442–#460).** Lee ruled a full re-review of the r3 waves ("we keep finding issues"): nine agents — discipline DISCIPLINED (0 flags, 15/15 landing check), both flow re-walks CONTINUOUS, live drive 8/8 PASS, task-menus adversary SOUND (94 checks), r3 engine adversary NOT SOUND with the round's headline: **cross-task fix_attempts contamination** (three independent live repros — `complete --next-task` pre-records current_task so the next `start` "resumes" a fresh task at attempts:3, while init's reset killed the true resume path anyway). Every r3 DESIGN decision survived attack; what broke was fix composition. Consolidated batch #459 (15 items): the **lockstep invariant** (counter+file always describe current_task's in-flight work; start owns current_task, complete zeroes, init preserves the live pair; composition tests), brief_path canonical-only rename, anchor gate over any per-phase item, map_total everywhere, honest review_cycles, treeHeader wrapping (same class found in epic phase blocks), spec-restart status resets, deep-dive conclusion drain, + small cluster. Also this round: **#458 task-gate menus codified** (labelled deferred-emission engine sections — new grammar in CONVENTIONS; two Lee rulings: code-commit tools declared "that's the whole point — to write code!", two-prompt gate menus blessed) and **#460 dynamic menus codified** per Lee's "why wait?" — 11 state-derived sites (inbox pickup/set/archived, manage availability logic, completed, absorb targets, empty state, view-plan topics, revisit menus × 8 files — killing the unfiltered-phase-list class permanently); ~60 fixed-vocabulary menus stay prose by ruling; select-* unit lists flagged follow-up. Stack-sync forensics closed: root cause was OUR stray `checkout||reset --hard` fallback poisoning api/read-side-reorg (evidence: reflog 15:24:59); tool verdict misuse-plus-amplifiers (silent empty-replay force-push, success-over-destruction, non-atomic undo); eval at ~/Downloads/stack-cli-eval.md; sync BANNED until `stack merge`; pr-stacked skill carries the preflights. **Post-launch queue: tick/local-markdown backend drivers (linear stays prose — MCP unreachable from subprocess); select-* unit-list codification.** Final targeted verification: **SHIP — nothing above LOW** (35/35 lockstep composition matrix incl. hostile inputs, 25/25 map-op checks, 33 dynamic-menu renders with clean option-key parity, 184/185 grammar checks, gates 1355/49/5 green). Two recorded LOWs for fix-forward, both unreachable-or-cosmetic: stale tracking-file adoption on a handover onto a previously-attempted id (prose flow can't produce it; threshold strictly conservative), and a dangling empty-state menu in start-from-inbox A's emission order. **THE STACK IS COMPLETE AND CERTIFIED: 57 PRs, ready for Lee's review → `stack merge` (sync stays banned) → launch fix-forward.**

**Relaunch docs (2026-07-19, close of the stack): #461 — THE STACK IS COMPLETE AT 58 OPEN PRs (#380, #383–#435, #442–#461).** README relaunch reached its final form through research (ten exemplar tool READMEs + spec-kit/BMAD/oh-my-zsh for the install-into-agent category; pattern brief at scratchpad/readme-research.md) and then LINE-LEVEL IN-CHAT review with Lee after three agent drafts failed his bar — the working method that landed: full draft in chat, his edits, commit only what survives. Final form: approved hook ("no memory and no process. This adds both."), buzzword-anchored selling copy with mechanism as proof (Real memory / A loop you can leave alone / Spec-driven by enforcement; multi-agent, context engineering, self-healing woven into sections), menus as fenced transcripts (blockquote menus broke on GitHub), no setup section beyond "there is no setup procedure", no agent-facing dumps (Lee: script dumps are implementation details), capture described as usage ("say log that as an idea"). docs/: 19-page site, reading-order index, history + dated timeline from git + this log, deep-research mandate (full closure reads before writing), authoring meta-commentary swept ("Ground rules" leak caught by Lee). Rulings recorded: em dashes banned from prose everywhere (renders exempt); nav labels must be universally understood; README register = engineer-to-engineer, never pitched down. **Next: Lee's stack review → `stack merge` (sync banned) → launch fix-forward.**

**Merge-day conventions pass (2026-07-20): #462–#463 — STACK FINAL AT 60 OPEN PRs.** Lee's final diligence ruling ("we usually find something") vindicated again: territory 1 (all 284 skill prose files, self-tested mechanical checker + ~70 full reads) fixed 40 files of surviving drift — em-dash rule in menus, canonical reference headers, marker-without-signpost ×8 (incl. workflow-start), command-value branch labels, Load link text, STOP variant, menu ordering. Territory 2 (23 briefs + every projection surface driven live + the rulebook itself): a real crash guard (spec completed-menu empty case), and CONVENTIONS.md corrected where it contradicted its own examples (menu shapes, missing `deferred`, deferred-section attribution, Key-in-DISPLAY note, mixed-prompt sample violating its own rule — sample + 2 prose mirrors aligned). All renders verified at exact widths, byte-exact frames, zero broken gutters. Flags for post-merge: epic Key full-vocabulary shape (documented pre-engine exception), Rules/Hard Rules brief-heading variance, out-of-contract gateway verbs' stack traces. Gates at tip: 1356 node, 328+ CLI, 49 migrations, typecheck. Final preflight over the full 59-link chain: ALL CLEAN — no inversion, no empty replay, no collapse. **Handed to Lee for `stack merge`.**

## Deferred ledger (CLOSED by PR 22 (#414) — kept for the record)
...
6. ~~RESOLVED in PR 21~~ Reindex walk exists twice after PR 20 — engine (`workunit reactivate`) and prose (`reindex-work-unit.md`, still loaded by pivot). Unify when pivot's chain migrates (or if `workunit pivot` is built).
7. ~~RESOLVED in PR 21~~ Seeds are not re-indexed on cancel→reactivate (prose gap carried faithfully — seed chunks lost under both writers). Fix alongside #6.
8. KB setup gate — **RULING REVERSED by Lee 2026-07-17**: the hard stop STAYS (setup is a deliberate human choice; auto keyword-only breeds permanently-unconfigured projects). Only the companion shipped: boot commits KB dirt it finds/creates, and (approved rule) every engine commit also stages `.workflows/.knowledge` — kills the orphaned-store-files problem after setup and everywhere else.

**Decisions ruled by Lee (2026-07-17), building in PR 21/22:** `topic supersede <wu> <phase> <topic> --by <topic>` with `superseded_by` = topic (handoff prose corrected); `workunit absorb` + `workunit pivot` as whole transactions (pivot retires reindex-work-unit.md, resolving #6; seeds fix #7 rides the walk change); KB commits per the every-engine-commit rule; planning review agents → no-git-writes contract. Standing rule change: obvious prose gaps are FIXED and flagged, not carried (see fix-gaps-dont-carry memory).
