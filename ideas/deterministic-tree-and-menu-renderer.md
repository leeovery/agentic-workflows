...
8. KB setup gate (Lee, 2026-07-17): the boot-time terminal stop on an uninitialised knowledge base exists only because interactive `setup` is the sole store creator. Fix: boot auto-initialises keyword-only (non-interactive) and continues with an upgrade note; `setup` becomes optional. Companion: boot commits KB dirt it finds/creates, and (approved rule) every engine commit also stages `.workflows/.knowledge` — kills the orphaned-store-files problem after setup and everywhere else.
