...
const records = [
  ...
  { record: 'assistant', text: 'discussing' },
...
describe('Replayer', () => {
  it('pauses at each user-record boundary and ends paused at the FINAL one', async () => {
    const r = new Replayer(makeFixture(records), { paceMs: 0 });
    const pauses: any[] = [];
    r.on('paused', (p) => {
      pauses.push(p);
      if (!p.final) setTimeout(() => r.step(), 5);
    });
    await r.run();
    expect(pauses.map((p) => p.final)).toEqual([false, true]);
    expect(r.status().state).toBe('paused'); // never 'ended' — Phase 2 adopt continues
  });

  it('offline mode consumes scripted answers and never pauses mid-stream', async () => {
    const r = new Replayer(makeFixture(records, { 'turn:2': { answer: '2', matchMode: 'key' } }), {
      paceMs: 0,
      offline: true,
    });
    const pauses: any[] = [];
    ...
    await r.run();
    expect(pauses).toEqual([{ atTurn: 3, final: true }]);
    expect(r.status().state).toBe('paused');
  });

  it('offline mode fails the run on an answer mismatch (exact)', async () => {
    const r = new Replayer(makeFixture(records, { 'turn:2': { answer: '3', matchMode: 'exact' } }), {
      paceMs: 0,
      offline: true,
    });
  ...
  it('streams every record before the final boundary', async () => {
    const r = new Replayer(makeFixture(records), { paceMs: 0 });
    const seen: string[] = [];
    ...
    await r.run();
    // Everything up to but excluding the final user record streamed.
    expect(seen).toEqual(['meta', 'user', 'assistant', 'turn-end', 'user', 'assistant', 'turn-end']);
  });
});
