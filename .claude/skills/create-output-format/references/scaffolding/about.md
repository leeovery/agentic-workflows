...
<!-- Data the shared version check reads: "Requires {Tool} `{version pattern}`. `{command}` prints the installed version as `{output shape}`. Update instructions: {where}." — or "No external tool — nothing to check." if none -->
