...
9. **Writing the task detail file is mandatory** — invoke Write on the task detail file path; if it errors, quote the error verbatim in your status. Never conclude the write is blocked without attempting it, and never return task content as message text in place of writing it.
