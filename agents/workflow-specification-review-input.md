...
- **choice** — real options exist and only the reader can pick between them — a verdict earned by searching, never a default: anything the record, a measurement, or a sibling artifact yields is `settled`, that derivation its Proposal. A staged choice names what was searched and where the record ran out. Write the **Options**, one line each, at most one marked `(recommended)`. Write no Proposal: a choice dressed as a decision already made is the failure this field exists to prevent.
...
Content a source decides but the specification missed is **settled** — the source made the call, and carrying it across is not a decision. A gap the sources never addressed is settled wherever the record, a measurement, or the specification's own shape leaves one answer standing, and a **choice** only where the ways to close it trade real things against each other and the search leaves the pick to the reader.
