...
describe('start projections: empty state', () => {
  ...
  it('renders the bare empty overview and start menu with no inbox and no closed units', () => {
    ...
    assert.strictEqual(menu.rendered, [
      ...
      `${NB(18)}shape it`,
      '**`f/feature`**       → Single topic: (research → experiments →)',
      `${NB(18)}discussion → spec → plan → implement → review`,
      ...
      `${NB(18)}planning)`,
      '**`c/cross-cutting`** → (Research → experiments →) discussion → spec',
      `${NB(18)}(patterns or policies that inform other work)`,
    ].join('\n'));
    assert.deepStrictEqual(
