...
// The harvested epic with E1 mid-measurement and results visible: the
// design froze at the briefing, the run began, and the report holds the
// first twenty sessions scored. The world the amendment boundary's far
// side needs — any design change from here is a dated correction and a
// successor, never an edit or a re-score.
...
module.exports = {
  build(h) {
    ...
    h.write(`${created.dir}/report.md`, [
      ...
      '## Results',
      '',
      'Sessions s-1 through s-20 scored so far: 12 of 20 zero-result',
      'searches recovered via an in-session reformulation with a click.',
      'Counting continues through s-40; every number traces to',
    ...
    h.engine('commit', WU, '--topic', `experiment/${TOPIC}`, '-m',
      `experiment(${WU}/${TOPIC}): E1 — first twenty sessions scored`);
  },
};
