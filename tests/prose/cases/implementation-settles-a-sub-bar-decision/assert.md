...
7. all three analysis agents dispatch in parallel with the same inputs
   ...
   webhook and the invisible unmatched capture. One returns findings,
   so the all-clean arm never applies: the findings commit lands scoped
   to the implementation topic, and the bank is read and prints empty
8. the synthesizer dispatches with no banked residue and stages two
   ...
   acknowledge a miss), and the unmatched-capture surfacing proposal
   carrying a **Decision** line, a **Stakes** line, and two sides
   written as product end states with the first marked (recommended).
   The gate state initialises in ONE batched write — both rows pending
   — and the synthesis commit lands
...
- the staging file's second task ends with a Solution carrying the
  ...
  in this walk — and the file holds no **Decision** and no **Stakes**
  line anywhere; both tasks carry the bodies the author added, and
  neither carried them before the walk
