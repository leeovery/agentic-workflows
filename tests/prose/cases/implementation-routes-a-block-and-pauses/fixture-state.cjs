...
// Between them they decide what a capture does to an order that is
// waiting for it, and what happens to a capture naming an intent no
// order carries. Neither reaches a capture that lands on an order the
// shopper has since cancelled: a cancelled order still carries its
// intent, and nothing says whether the order comes back or stays gone.
// That is the fork the executor stops on, and the user's answer sends it
// to the discussion rather than settling it at the keyboard.
