...
1. the entry reads the discussion status — in-progress, reopened by
   the triage delivery — gathers context from the topic's brief, and
   hands off into the processing skill
...
- the two amended sites (the Expansion Source Journey and the Summary
  Current State bullet) each carry a dated note naming
  behavioural-ranking's decision; "click-weights table" survives
  nowhere as the current mechanism — at most inside an amendment note
  recording its retirement
- the Expansion Source Decision block is untouched in substance: the
