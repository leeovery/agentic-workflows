...
// opts into auto), a choice the record in fact settles — the discussion
// ties the total's re-quote to exactly one event, the customer's hand
// edit, so a re-render without one is not that event — and a choice the
// sources leave genuinely open. The session must dispose the middle one
// to settled and apply it under auto with no stop, and let the third
// stand and stop. The non-recommended option is the one the user picks
// there, so the end state distinguishes their choice from any
// rubber-stamp of the recommendation.
...
module.exports = {
  build(h) {
    ...
    // total decided with its one re-quote tied to the hand edit; the
    // failed-webhook retry ceiling never raised.
