...
// with two gaps on decided ground, and the user walks them — each
// settled as an amendment to the decision it extends, never new
// ground — so the map stays settled while the close is interrupted,
// and the close resumes by itself once the walk drains.
