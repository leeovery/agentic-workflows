...
// The fixture plus the peer's re-landing — performed mid-walk by the armed
// substitution — and the one thing the session under test should have
// done: the reconcile flag consumed by the advisory once its own check
// found the research landed. Nothing else moves — the walk stops at the
// session's first turn.

const fixture = require('./fixture-state.cjs');

const { WU, TOPIC } = fixture;

// The research file as the peer leaves it — byte for byte the content of
// tests/prose/stubs/research-peer-relanding.md, which the walker writes;
// this copy is what the recipe hash covers.
const RELANDED_RESEARCH = [
