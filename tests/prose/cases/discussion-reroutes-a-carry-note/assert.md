...
1. the entry reads the discussion status, finds it in progress, emits
   ...
   silent), reads the map item's source and gathers nothing — asking
   the user nothing — and hands off with source: existing discussion
...
5. the store holds no review row, so the review gate is mandatory; the
   user proceeds; the final gap review dispatches the review agent and
   the stubbed report comes back clean — the row incorporates with
   nothing to raise
6. document review re-reads the document and the map, and the
   reconciliation finds the conversation faithfully recorded — except
   the misdirected-knowledge dimension: the Open Threads carry-note
   addressed to behavioural-ranking is knowledge owed to a decided
...
- no research or discussion session was opened on behavioural-ranking
