...
// A specification at the review boundary whose input review returns both
// moves: two settled calls the discussion determines, and one genuinely
// open choice. The user opts into auto at the first finding, so the
// second must land with no stop — and the third must stop anyway,
// because auto never decides what only the user can. The non-recommended
// option is the one they pick, so the end state distinguishes their
// choice from any rubber-stamp of the recommendation.
