...
// The harvested epic with the synonym-handling research mid-flight. The
// file has narrowed the replacement question to two candidate sources and
// names the number the choice turns on — the in-session recovery share of
// zero-result searches — as unmeasured. A day's sandbox session log sits
// in the repo: the real thing an experiment would measure.
...
module.exports = {
  build(h) {
    ...
    h.write(`.workflows/${WU}/research/${TOPIC}.md`, [
      ...
      'Nobody has measured it. A day of sandbox search-session activity is',
      'exported at logs/search-sessions.log.',
      '',
    ].join('\n'));
