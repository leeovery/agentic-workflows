...
10. the compliance self-check refreshes the session's instructions, the
    conclude gate is put to the user and, on their yes, the plan
    completes through the engine first and the spec baseline is
    re-stamped after it, the final commit lands, and the walk stops at
    the pipeline continuation without invoking the bridge
