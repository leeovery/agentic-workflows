...
1. the entry reads the discussion status, finds it in progress (the
   triage reopen left it so), emits the resuming phase note, checks the
...
- the `### Decision` block under `## Expansion Source` holds two
  entries, latest first: a dated `#### {date} — revised` entry
  directly beneath the heading, then `#### Initial` below it. The
  revised entry's trigger line names the substance of what prompted
  it — the live click-signal stream will not be built / the
  batch-only decision from behavioural-ranking — never just a cache
  or finding id
- the current decision (the top entry) records expansion computed
