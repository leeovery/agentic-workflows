...
target could accept or reject independently, landed pre-split in one
file. The delivery created the discussion item as `triaged`; no
artifact file exists, no session has ever run, and the one-entry
queue is the entire agenda the next session inherits.
