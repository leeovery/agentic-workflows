...
// then `v2`, each holding one waiting item. That is what makes the
// horizon a pick rather than a question, and what makes the postpone
// join an existing map rather than create one.
...
module.exports = {
  build(h) {
    ...
    // The product roadmap, laid out before this epic was pulled together.
    h.engine('roadmap', 'add', 'saved-searches', '--horizon', 'v1',
      '--summary', 'shoppers keep a search and hear when its results change');
    h.engine('roadmap', 'add', 'merchandising-rules', '--horizon', 'v2',
      '--summary', 'buyers pin and bury products for a campaign');
  },
};
