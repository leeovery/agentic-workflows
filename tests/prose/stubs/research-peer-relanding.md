# stub: research-peer-relanding

A peer research session's re-landing, performed mid-walk. The walker
performs this once, acting as the peer session named in the case's
arming — never as the session under test: run the reopen the arming
names, write the content below over the research file the arming
names, then run the completion and the commit the arming specifies.
The engine flags the discussion at the reopen and releases the wait at
the completion; from that moment the landed research is ordinary world
state the session under test discovers through its own checks.
