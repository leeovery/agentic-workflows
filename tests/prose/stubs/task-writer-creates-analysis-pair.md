...
1. Create `.workflows/pay/planning/pay/tasks/pay-2-1.md` and
   `.workflows/pay/planning/pay/tasks/pay-2-2.md` for the two approved
   tasks in staging order, each with the frontmatter below (its own
   `id`) and a body carrying that task's title as the `#` heading, its
   **Do** content as the description, and its
